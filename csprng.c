...
#define VERSION 4121 //id for the validation
#define FILTER_TYPE 7// 0=> RSG overworld (Practice Seeds); 1=> village only; 2=> shipwreck only; 3 => jungle only; 4 => coinflip (classic); 5=> whatever I want; 6 => loot testing; 7 => Flower Spring Greatness
#define DEBUG 0
#include <string.h>
#include <time.h>
...
char netherchecker(int64_t seed, int* fortressQuadrant){
  ...
  *fortressQuadrant = 0;
  ...
  if (chunkx <= 12 || chunkz <= 12 || structureType == 0){
    return 0;
  }
  int gotfort = 0;
  ...
  if (chunkx >= 15 || chunkz <= 12 || structureType == 1){
    return 0;
  }
  ...
  if (chunkx > 12 || chunkz < 16 || structureType == 1){
    return 0;
  }

  fakeseed = (seed + 30084232UL - BB - AA) ^ 0x5deece66dUL;
  chunkx = next(&fakeseed, 31) % 23;
  chunkz = next(&fakeseed, 31) % 23;
  structureType = (next(&fakeseed, 31) % 5)  >= 2;
  bastionCount += structureType;
  ...
  if (chunkx < 16 || chunkz < 16 || structureType == 1){
    return 0;
  }
...
char bastionbiome(uint64_t seed){
  ...
  NetherBiomes biome2=get_biome(netherGen,chunkx*16 + 8,32,chunkz*16 + 8);  
...
int flowerBiome(int64_t seed, LayerStack* gp){
  int mc = MC_1_16;
  Pos p = getSpawn(mc, gp, NULL, seed);
  int biomeAtPos = getBiomeAtPos(gp, p);
  if (biomeAtPos ==  flower_forest ||  biomeAtPos == sunflower_plains || biomeAtPos == plains){
    return 1;
  }
  return 0;
}
...
//        seed = 7086636929918596711LL;
    ...
    printf("Happy April! Tell us your best times on these sick seeds in the Java Speedrunning Discord!\n");
    return 0;
}